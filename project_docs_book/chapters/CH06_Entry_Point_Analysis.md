...
- You are here: Chapter 6
- Before this: Chapter 5
- After this: Chapter 7

## Learning Objectives
- Trace how this backend starts in local and deployed environments.
- Identify runtime entrypoints for CLI, WSGI, and ASGI.
- Understand first request dispatch from process start into app routes.
...
## Entry Point Types in This Project
There are three concrete entrypoint categories:

1. CLI/Admin entrypoint
- File: `manage.py`
- Behavior:
  - Sets `DJANGO_SETTINGS_MODULE=django_project.settings`.
  - Calls `django.core.management.execute_from_command_line(sys.argv)`.
- Used for commands like `runserver`, `migrate`, `collectstatic`, custom management commands.

2. WSGI web entrypoint
- File: `django_project/wsgi.py`
- Behavior:
  - Sets `DJANGO_SETTINGS_MODULE=django_project.settings`.
  - Exposes `application = get_wsgi_application()`.
- Production startup uses this entrypoint through Gunicorn:
  - `gunicorn -c gunicorn.conf.py django_project.wsgi:application`

3. ASGI web entrypoint
- File: `django_project/asgi.py`
- Behavior:
  - Sets `DJANGO_SETTINGS_MODULE=django_project.settings`.
  - Exposes `application = get_asgi_application()`.
- ASGI file is present, but deployment commands in this repository currently point to WSGI/Gunicorn.

## Deployment Startup Flow (Observed)
### Procfile path
...
- Start command:
  - `python manage.py migrate && python manage.py bootstrap_admin && gunicorn -c gunicorn.conf.py django_project.wsgi:application`

This means process boot order is:
1. environment + dependencies ready,
2. Django management commands run,
3. Gunicorn launches workers,
4. workers load `django_project.wsgi:application`.

## First Request Dispatch Path
After Gunicorn starts WSGI app:
1. Django loads settings from `django_project.settings`.
2. `ROOT_URLCONF` points to `django_project.urls`.
3. Incoming request passes middleware chain (includes `common.middleware.LegacyApiDeprecationMiddleware`).
4. URL routing resolves endpoint:
   - `/health/`, `/admin/`, auth/schema/docs routes, or app routes under `/api/` and `/api/v1/`.
5. App-level router/viewset handles the request.

## Runtime Server Configuration Entry
`gunicorn.conf.py` defines runtime behavior:
- bind address/port,
- worker count,
...
This file is part of the effective entry pipeline because startup command directly references it.

## Additional Entry-Relevant Signals
- Render health check path is `/health/` and is mapped in `django_project/urls.py`.
- JWT token endpoints are direct entry APIs for authenticated usage:
  - `/api/auth/token/`
  - `/api/auth/token/refresh/`
  - versioned equivalents under `/api/v1/`.

## What Is Not Present
- No alternate production command using ASGI server (for example `uvicorn` or `daphne`) is configured in Procfile/render file.
- No separate worker process entry command (for example Celery worker) is configured in deployment files.

## Chapter 6 Outcome
You now have a full startup-to-request entry map: command/process entry, WSGI boot, middleware pass, root URL dispatch, and app endpoint handling.
```
